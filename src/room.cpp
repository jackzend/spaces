...
void Room::setNeighbor(std::shared_ptr<Room> nbor, SpacesNeighbors i)
{
   if (is_uninitialized(_mNeighbors[static_cast<size_t>(i)]))
   {
      ...
      case SpacesNeighbors::LEFT:
         ...
         nbor->setOrigin(this->_mLocation.x - SPACES_ROOM_WIDTH, this->_mLocation.y);
         break;
      ...
      case SpacesNeighbors::UP:
         ...
         nbor->setOrigin(this->_mLocation.x, this->_mLocation.y - SPACES_ROOM_HEIGHT);
         break;
